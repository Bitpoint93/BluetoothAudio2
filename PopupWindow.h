...
static constexpr float kHeaderH       = 44.f;   // icon + "BT Audio Receiver"
static constexpr float kConnectedH    = 72.f;   // green dot + name + "Connected"
static constexpr float kSliderH       = 48.f;   // volume row
static constexpr float kDividerH      = 1.f;
static constexpr float kSectionLabelH = 24.f;   // "OTHER DEVICES"
static constexpr float kDeviceRowH    = 44.f;
static constexpr float kFooterH       = 52.f;   // Settings + Disconnect
static constexpr float kCornerRadius  = 12.f;
static constexpr int   kMaxOtherDevices = 5;    // never show more than this many
...
public:
    ...
    // Called from the host window when BT state changes
...
private:
    ...
    // Volume
    float m_volume = 1.f;          // 0..1
    bool  m_draggingSlider = false;
    void  SetVolume(float v);

    // ── data (refs owned by main.cpp) ────────────────────────
    const std::unordered_map<std::wstring, std::wstring>& m_devices;
    const std::wstring&                                    m_connectedId;
    std::function<void(const std::wstring&)>               m_fnConnect;
    std::function<void()>                                  m_fnDisconnect;

    // Sorted list of "other" device ids (not connected)
    ...
    // Cached layout Y positions for hit-testing (set during Render)
    float m_sliderY   = 0;
    float m_devicesY0 = 0;   // first device row top
    float m_footerY   = 0;
    float m_winH      = 0;
    ...
    // Brushes (recreated per render if color scheme changes)
    ...
    ComPtr<ID2D1SolidColorBrush>    m_brushAccent;     // blue
